...
type Product = {
    name : string ,
    allothercolorprice : number ,
    onlynewcolorprice?:number, 
    newcolor?:string,// price adjusted for color change
...
let products :Product []= [{name : "suzuki Mehran",allothercolorprice : 500000, inventory : {stock :4, colorOptions:["black","white","grey"]}},
...
function changeColor(product:Product,color:string,newprice=product.allothercolorprice){
    if(color === "red"){product.onlynewcolorprice = Math.round(1.1 * product.allothercolorprice);
        product.inventory.colorOptions.push(color);product.newcolor=color;
        product.specialinstructions="price increased due to color";
    }else if (color === "blue"){product.onlynewcolorprice = Math.round(0.95 * product.allothercolorprice);
        product.inventory.colorOptions.push(color);product.newcolor=color;
        product.specialinstructions="price decreased due to color" }; return product;
}
